...
if uploaded_file is not None:
    if start_date > end_date:
        ...
        def load_data(file):
            ...
            for col in date_cols:
                if col in df.columns:
                    # THÊM `dayfirst=True` ĐỂ LUÔN ƯU TIÊN ĐỌC ĐỊNH DẠNG NGÀY TRƯỚC, THÁNG SAU
        ...
        dates = {
            ...
            'report_start_date': start_date_ts,
            'report_end_date': end_date_ts
        }
        ...
        CHILD_COL_ORIGINAL = 'Đơn vị thực hiện KPCS trong quý'
        CHILD_COL_RENAMED = 'Đơn vị thực hiện KPCS trong kỳ'

        text_cols = [CHILD_COL_ORIGINAL, PARENT_COL, 'ĐVKD, AMC, Hội sở (Nhập ĐVKD hoặc Hội sở hoặc AMC)']
        ...
        if CHILD_COL_ORIGINAL in df.columns:
            df.rename(columns={CHILD_COL_ORIGINAL: CHILD_COL_RENAMED}, inplace=True)
            df = df[~df[CHILD_COL_RENAMED].str.lower().str.contains('tổng cộng|tổng', na=False)]
            CHILD_COL = CHILD_COL_RENAMED
        else:
             CHILD_COL = CHILD_COL_RENAMED # Giả sử nó có thể đã được đổi tên
